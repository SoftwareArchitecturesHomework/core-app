      ...
      transformedEvents = nyersTasks.map((task) => {
        ...
        if (startDate && endDate) {
          const isStartMidnight =
            startDate.getUTCHours() === 23 && startDate.getUTCMinutes() === 0
          const isEndEndOfDay =
            endDate.getUTCHours() === 22 && endDate.getUTCMinutes() >= 59
...
<template>
  <div class="p-5 bg-default rounded-xl shadow-lg text-gray-200 min-h-screen">
    <div class="flex justify-between items-center py-5">
      <div class="flex items-center gap-2 lg:gap-4">
        <UIcon
          name="i-heroicons-calendar-days"
          class="text-4xl lg:text-5xl transition-colors"
          :class="{
            'text-primary-400': !isWeeklyView,
            'text-gray-500': isWeeklyView,
          }"
        />
        <span
          class="text-lg lg:text-xl transition-colors"
          :class="{
            'font-semibold text-primary-400': !isWeeklyView,
            'text-gray-500': isWeeklyView,
          }"
          >Month</span
        >
    ...
    <div
      class="mt-4 bg-default p-2 rounded-lg shadow-inner text-default dark:text-gray-400 border-gray-200 dark:border-gray-700 dark:[--fc-border-color:var(--color-gray-600)]"
    >
      <FullCalendar
        class="[&_table]:rounded-md [&_table]:overflow-hidden [&_table]:border-gray-200! dark:[&_table]:border-gray-700! [&_thead_th]:first-of-type:rounded-tl-md [&_thead_th]:last-of-type:rounded-tr-md [&_tbody]:last:first:rounded-b-md"
    ...
    <EventCreationModal
      ...
      @created="eventCreatedOrCanceled()"
      @cancel="eventCreatedOrCanceled"
    ></EventCreationModal>
  </div>
</template>
