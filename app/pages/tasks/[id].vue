...
const { data: task, error, pending, refresh } = await useFetch(`/api/tasks/${route.params.id}`)
...
function getTaskTypeColor(type: TaskType): 'error' | 'info' | 'primary' | 'secondary' | 'success' | 'warning' | 'neutral' {
    switch (type) {
    ...
    <div class="p-6 max-w-7xl mx-auto">
        ...
        <UCard v-else-if="error">
            <div class="text-center py-12">
                ...
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">Error Loading Task</h3>
                <p class="text-gray-600 dark:text-gray-400">{{ error.statusMessage || 'Failed to load task details' }}
                </p>
        ...
        <div v-else-if="task">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <UCard>
                        <template #header>
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    ...
                                    <UBadge color="secondary" variant="soft">{{ task.timeEntries.length }}</UBadge>
                                </div>
                        ...
                        <div v-if="task.timeEntries.length > 0" class="divide-y divide-gray-200 dark:divide-gray-800">
                            <div v-for="entry in task.timeEntries" :key="entry.id" class="py-4 first:pt-0 last:pb-0">
                                <div class="flex items-start justify-between">
                                    <div class="flex items-center gap-3">
                                        <UAvatar :src="entry.user.image || undefined" :alt="entry.user.name || 'User'"
                                            size="sm" />
                                        <div>
                                            <p class="font-medium text-gray-900 dark:text-white">{{ entry.user.name }}
                                            </p>
                                            <p class="text-sm text-gray-600 dark:text-gray-400">{{
                                                formatDate(entry.date) }}</p>
                                            <p v-if="entry.note" class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                                {{
                                                    entry.note }}</p>
                                        </div>
                                    </div>
                                    <UBadge color="primary" variant="soft">{{ entry.hours }}h</UBadge>
                                </div>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-1 space-y-4">
                    <UCard v-if="task.project">
                        ...
                        <NuxtLink :to="`/projects/${task.project.id}`"
                    ...
                    <UCard v-if="task.type === 'TASK'">
                        ...
                        <div class="space-y-3">
                            ...
                            <UButton v-if="isAssignee || isCreator" :color="task.isDone ? 'info' : 'primary'" block
                                variant="ghost" @click="toggleTaskStatus">
                                <UIcon :name="task.isDone ? 'i-heroicons-arrow-path' : 'i-heroicons-check'"
                                    class="w-4 h-4" />
                    ...
                    <UCard>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label
                                    class="text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide">Creator</label>
                                <div class="flex items-center gap-2 mt-2">
                                    <UAvatar :src="task.creator.image || undefined" :alt="task.creator.name || 'User'"
                                        size="sm" />
                            ...
                            <!-- Assignee -->
                            <div v-if="task.type === 'TASK'" @click="openAssigneeModal" class="cursor-pointer group">
                                <label
                                    class="text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide">Assignee</label>
                                <div v-if="task.assignee"
                                    ...
                                    <UAvatar :src="task.assignee.image || undefined" :alt="task.assignee.name || 'User'"
                                        size="sm" />
                                    <div class="flex-1 min-w-0">
                                        <p class="font-medium text-sm text-gray-900 dark:text-white truncate">{{
                                            task.assignee.name }}
                            ...
                            <div v-if="task.type === 'MEETING'" @click="openAssigneeModal" class="cursor-pointer">
                                ...
                                <div v-else>
                                    <p class="text-sm text-gray-400">No participants added yet.</p>
                                </div>
            ...
            <AddTimeEntryModal v-model="timeEntryModalOpen" :taskId="task?.id" @timeEntryAdded="refresh" />
            <TaskAssigneeModal v-model="assigneeModalOpen" :taskId="task?.id" :projectId="task?.projectId"
                :currentAssigneeId="task?.assignee?.id" @assigneeUpdated="refresh" />
        </div>
    </div>
