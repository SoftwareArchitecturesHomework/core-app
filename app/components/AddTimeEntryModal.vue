...
async function onSubmit() {
    ...
    try {
        ...
        const errorMessage = error?.data?.statusMessage || error?.statusMessage || error?.message || 'Failed to add time entry'
        toast.add({
            title: 'Error',
    ...
    <UModal v-model:open="isOpen" title="Add Time Entry" size="md">
        <template #body>
            <div class="flex flex-col gap-4">
                ...
                <div class="space-y-2">
                    ...
                    <UInput v-model="newTimeEntry.hours" type="number" step="0.5" min="0" placeholder="e.g. 2.5"
                        class="w-full">
                        <template #trailing>
                ...
                <div class="space-y-2">
                    ...
                    <UTextarea v-model="newTimeEntry.note" placeholder="Add any notes about this work session..."
                        :rows="3" class="w-full" />
                </div>
            </div>
        </template>
        <template #footer>
            <div class="flex justify-end space-x-2">
                ...
                <UButton color="primary" @click="onSubmit" :loading="isSubmitting" :disabled="isSubmitting">
                    Add Time Entry
                </UButton>
            </div>
        </template>
